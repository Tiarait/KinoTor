 ...
 */
public class SearchKinolive extends AsyncTask<Void, Void, Void> {
    private String query;
    private List<ItemSearch> items;
    private OnTaskSearchCallback callback;

    public SearchKinolive(String query, OnTaskSearchCallback callback) {
        items = new ArrayList<>();
        this.query = query;
        ...
        if (data != null) {
            if (data.html().contains("</a>")) {
                ...
                for (Element entry : allEntries) {
                    ...
                    if (entry.html().contains("comganre")) {
                        description_t = entry.select(".comganre").last().text();
                    }
                    if (entry.html().contains("<img")) {
                        img = Statics.KINOLIVE_URL + entry.select("img").first().attr("src");
                    }
    ...
    private Document Getdata(String query) {
        try {
            return Jsoup.connect(Statics.KINOLIVE_URL + "/engine/ajax/search.php")
                    .data("query", query)
