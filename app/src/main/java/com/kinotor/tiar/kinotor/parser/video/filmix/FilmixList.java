...
public class FilmixList extends AsyncTask<Void, Void, Void> {
    ...
    public FilmixList(String url, OnTaskVideoCallback callback, boolean season, String trans) {
        this.url = url;
        this.season = season;
    ...
    public FilmixList(String url, OnTaskVideoCallback callback, boolean series, String trans, String s) {
        this.url = url;
        this.series = series;
        this.callbackVideo = callback;
        this.trans = trans;
    ...
    private void parseSeason(String doc) {
        ...
        doc = doc.replace(" ","");
        if (doc.contains("]},{") && doc.contains("title\":\"Сезон")) {
...
//        Log.e("test", "parseSeries: "+s );
        String season, episode, id = "error";
        if (s.contains("post_id\":\""))
            id = s.split("post_id\":\"")[1].split("\"")[0];
        if (s.contains("title\":\"Сезон"))
            season = s.split("title\":\"Сезон")[1].split("\"")[0];
        else season = "0";
        if (s.contains("title\":\"Серия")) {
            episode = s.split("title\":\"Серия")[s.split("title\":\"Серия").length - 1]
                    .split("\\(")[0];
        } else episode = "0";
//        Log.e("test", "parseSeries1: "+episode );

        if (episode.contains("\""))
            episode = episode.split("\"")[0].trim();
        if (episode.contains("-"))
...
//        Log.e("test2", "parseSeries: "+s );
        ...
        if (season.equals(this.se) && s.contains("title\":\"Серия")) {
            String[] curSe = s.split("title\":\"Серия");
            for (int i = 1; i < curSe.length; i++) {
//                Log.e("test3", "parseSeries: "+curSe[i] );
                String ep = curSe[i].split("\"")[0].trim();
                if (ep.contains("("))
                    ep = ep.split("\\(")[0].trim();

                String u = curSe[i].contains("file\":\"") ?
                        curSe[i].split("file\":\"")[1].split("\"")[0]
                                .replace(",,,,,","")
                                .replace(",,,,","")
                                .replace(",,,","")
                                .replace(",,","").replace(" or ","or") : "error";
