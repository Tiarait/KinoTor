 ...
 */
public class SearchKoshara extends AsyncTask<Void, Void, Void> {
    private String query;
    private List<ItemSearch> items;
    private OnTaskSearchCallback callback;

    public SearchKoshara(String query, OnTaskSearchCallback callback) {
        items = new ArrayList<>();
        this.query = query;
    ...
    private Document Getdata(String query) {
        try {
            return Jsoup.connect(Statics.KOSHARA_URL + "/engine/ajax/search.php")
                    .data("query", query)
