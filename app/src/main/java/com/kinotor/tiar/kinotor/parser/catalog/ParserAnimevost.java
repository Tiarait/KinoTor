    ...
    private void ParseHtml(Document data) {
        if (data != null) {
            ...
            if (data.html().contains("class=\"shortstory\"") && !data.html().contains("id=\"comment\"")) {
                ...
                for (Element entry : allEntries) {
                    if (entry.html().contains("shortstoryHead")) {
                        ...
                        if (title.contains("/")) {
                            ...
                            subname = subname.replace("Second Season","");
                ...
                if (data.body().html().contains("itemprop=\"description\"")) {
                    description = data.select("span[itemprop='description']").html();
                } else if (data.body().text().contains("Описание: ") && data.html().contains("shortstoryContent"))
    ...
    private Document Getdata(String url) {
        try {
            ...
            if (url.contains("'page'")) {
                htmlDoc = Jsoup.connect(url.split( "'page'")[0]+"/index.php?do=search")
                        .data("do", "search")
